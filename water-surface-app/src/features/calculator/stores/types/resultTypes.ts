...
import { 
  ...
  export function getProfileTypeDescription(profileType: ProfileType): string {
    return PROFILE_TYPE_DESCRIPTIONS[profileType];
  }
  
  /**
   * Convert flow depth points to calculation results
   * This function is used to convert between the internal calculation format
   * and the format displayed in the UI
   * 
   * @param flowProfile Array of flow depth points
  ...
  export function convertFlowPointsToResults(
    ...
    _channelParams: any // Using any to avoid circular import, would be ChannelParams
  ): CalculationResult[] {
    return flowProfile.map(point => ({
      station: point.x,
      depth: point.y,
      velocity: point.velocity,
      area: 0, // Would need to calculate from channel geometry
      topWidth: 0, // Would need to calculate from channel geometry
      wetPerimeter: 0, // Would need to calculate from channel geometry
      hydraulicRadius: 0, // Would need to calculate from channel geometry
